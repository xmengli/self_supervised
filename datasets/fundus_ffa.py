...
class traindataset(data.Dataset):
    ...
    def __init__(self, root, transform=None, train=True, args=None):
        ...
        test_path = [self.root_dir + "FFA/normal/" + str(item)+ "-" + str(item) + ".jpg" for item in list(range(1, 31))]
        test_path += [self.root_dir + "FFA/abnormal/" + str(item)+ "-" + str(item) + ".jpg" for item in list(range(1, 31))]
        label_list_test = [0 if item.split('/')[-2] == "normal" else 1 for item in test_path]
    ...
    def __getitem__(self, idx):
        ...
        if self.train and self.multiaug:
            ...
            img_syn = self.train_syn[idx]
            img_syn = Image.fromarray(np.uint8(img_syn))
            img_syn = self.transform(img_syn)

            return [img, img2, img_syn], [target], idx, self.name[idx]
